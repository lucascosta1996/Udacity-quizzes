...
function lessThan(album){
    return (album.artist + album.name).length < 25;
}

var filtered = musicData.filter(lessThan);

const totalAlbumSales = filtered.reduce((acc, currentVal) => {
    return acc + currentVal.sales;
}, 0);
